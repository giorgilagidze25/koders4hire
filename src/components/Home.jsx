...
export default function Home({ darkMode }) {
  ...
  const handleDelete = async (id) => {
    ...
    try {
      ...
      if (res.status === 200) setServices((prev) => prev.filter((s) => s._id !== id));
      else if (res.status === 403) alert("Not authorized");
      else if (res.status === 404) alert("Service not found");
        ...
        {services.length > 0 ? (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
            {services.map((service) => {
              const isOwner = currentUser && (service.owner?._id === currentUser._id || service.author?._id === currentUser._id);
              return (
                <div
                  key={service._id}
